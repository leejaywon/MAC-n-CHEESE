...
## 0. 우승 전략 (과거 우승작 3건 분석 기반)

과거 우승 축: **자율성(git zero-touch) > 하네스 설계 > 비용 규율**. 1위 제품은
방청소 봇으로 평범했고, 3위는 $0.55 비용으로 입상. 심사는 결과물 화려함이 아니라
과정을 본다. 이번은 연구 에디션이라 "논문·리뷰 품질"이 추가되지만 이벤트 명시상
**agent workflow(하네스)도 심사 대상**.

우리 3대 승부 카드 (심사 피치 순서):
1. **Zero touch 자율성** — `git log`에 사람 커밋 0. 랍스터 옷 한 번도 안 입음.
2. **폐쇄 자기검증 루프** — 실험→증거원장→논문→리뷰 게이트(자기 감사기 통과해야
   제출). 이번 테마("better research loops")의 문자 그대로의 답. 우리만의 카드.
3. **비용 규율** — 캠페인 총액 ~$5(A100) + Track 2 거의 $0. **데모에서 비용을
   숫자로 전면에 낸다** (3위가 $0.55로 입상한 신호). 매 iteration의 `tokens: total`
   과 `vesslctl billing show` 델타를 캡처해 최종 슬라이드에 박을 것.

우리 최대 리스크 = **Track 1 차별화 얇음**(모두 같은 Training path). 대응: 승부를
Track 2 품질 + 폐쇄 루프 스토리에 싣는다. Track 1은 "정직한 결과 + 자기검증 통과"
로 충분, 화려함 경쟁 안 함.

Dalpha 특별상(world-model, 미확정): 우리 Karpathy Track 1은 해당 안 됨(generic
predictor를 world model이라 부르면 자기 리뷰어가 잡음). 팀원 여유 계정 있을 때만
General Track 1로 값싼 world-model 실험 사이드 헤지. 메인 파이프라인은 불변.
...
(`printenv OPENAI_API_KEY | codex login --with-api-key` — CLI 0.144.1 확인함;
`--api-key` 플래그는 없어졌다. 키 입력은 사람이), 아니면 구독 유지.
...
**데모 (5–7분 예상)** — 스토리는 시스템 두 개가 아니라 **"자기검증 연구 루프
하나"**: 실험(A100) → 증거 원장 → 페이퍼 → 리뷰 게이트(자기 감사기를 통과해야
제출) → 사람 손 0. 순서: ①한 문장 프레임("우리 논문은 우리 리뷰어를 통과해야
세상에 나옵니다") ②W&B 차트(캠페인 val_bpb + 리뷰어 eval) ③git log 스크롤(사람
커밋 0) ④리뷰어 라이브: 오류 주입 페이퍼 + **외부 실물(Sakana 공개 AI 논문)에서
...
| rate limit / usage limit | **전날 실증: sol-high ~25 iteration에 구독 쿼터 소진, terra도 같은 계정 쿼터.** 3시간 루프 2개는 구독 하나로 불가 → 아침에 크레딧 받으면 `codex login --with-api-key`(키 stdin)를 최우선으로. 크레딧 없으면 팀원 계정에 한 루프 배정. 루프는 연속 실패 3회에 자동 정지하게 해뒀음 |
